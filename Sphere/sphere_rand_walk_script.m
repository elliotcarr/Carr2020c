%% 3D Lattice free sphere random walk
clear all; close all; clc
% This script simulates a multi-layer random walk on a sphere/spherical shell in 3 dimensions
...
%P = [0.6]; % Probability of each layer
Case = '1';
% Case = '2';
% Case = '3';
if strcmp(Case,'1')
...
delta = 1; % step size in radial direction
tau = 1; % time step
start_radius = R(1):10:R(end); % Start radius as a vector of values to try
% start_radius = R(1); % Start radius or a vector of test radii
start_theta = pi/4; % Start angle (radians)
start_phi = pi/4; % Starting elevation angle
...
% Initialise
if exist('P','var') == 1
    D = P*delta^2/(6*tau); % Diffusivities
    if any(P>1) || any(P <= 0)
        error('Invalid probabilities defined');
    end
elseif exist('D','var') == 1
    P = 6*D*tau/(delta^2); % Probabilities
    maxD = delta^2/(6*tau);
    if any(D > maxD) 
        error(['Please choose 0 < D <= ' num2str(maxD)]);
    end
else
    error('Please define D or P')
end
% Divide spherical step into new coordiantes
[tempA,tempB] = meshgrid( 2*pi*linspace(0,1,7),-pi/2 + acos(2*linspace(0,1,7)-1) );
% [tempA,tempB] = meshgrid( 2*pi*(0:0.2:1),-pi/2 + acos(2*(0:0.2:1)-1) );
meshA = tempA(2:end-1,1:end-1); meshB = tempB(2:end-1,1:end-1); % Remove poles and overlapping periodic points
[n1,n2] = size(meshA); % Create spherical mesh
partitions(1,:) = reshape(meshA,[1,n1*n2]);
partitions(2,:) = reshape(meshB,[1,n1*n2]);
partitions = [partitions [ 0 0; pi/2 -pi/2] ]; % Add in the poles of the sphere
% Initialise arrays
...
parfor ii = 1:length(start_radius)
    ...
    exit_times = sphere_rand_walk_func(P,R,delta,tau,partitions,...
        configuration,start_radius(ii),start_theta,start_phi,sim_num);
    for jj = 1:sim_num
...
if save_data == true
    save(['sphere_data_D_' temp_str1 '_Radii_' temp_str2 '_dir_' ...
...
for moment_choice = moments_to_plot
    ...
    y = moments(R,D,configuration,3,R(1):delta:R(end),moment_choice);
    ...
    if save_plots
        if strcmp(Case,'1')
            if moment_choice == 1
                ...
                drawnow
                feval('export_fig',[file_path,'Figure2a_SI.pdf'],'-pdf')
            elseif moment_choice == 2
                ymax = 6e9;
                ylim([0,ymax]) 
                ...
                drawnow
                feval('export_fig',[file_path,'Figure2d_SI.pdf'],'-pdf')
            end
        elseif strcmp(Case,'2')
            if moment_choice == 1
                ymax = 18e4;
                ...
                feval('export_fig',[file_path,'Figure2b_SI.pdf'],'-pdf')
                drawnow
            elseif moment_choice == 2
                ymax = 4.5e10;
                ylim([0,ymax])                 
                ...
                feval('export_fig',[file_path,'Figure2e_SI.pdf'],'-pdf')
                drawnow
            end
        elseif strcmp(Case,'3')
            if moment_choice == 1
                ...
                feval('export_fig',[file_path,'Figure2c_SI.pdf'],'-pdf')
                drawnow
            elseif moment_choice == 2
                ymax = 10e8;
                ylim([0,ymax])         
                ...
                feval('export_fig',[file_path,'Figure2f_SI.pdf'],'-pdf')
                drawnow
            end
        end
    end
...
exit_times_eff = sphere_rand_walk_func(P,R,delta,tau,partitions,...
        configuration,start_radius_eff,start_theta,start_phi,sim_num);
...
text(0.7,0.75,['$d = 3$ (sphere)'],'Units','Normalized',...
    'FontSize',30,'Interpreter','LaTeX')
text(0.7,0.5,['$D_{\mathrm{eff}} = ',num2str(Deff,'%.4f'),'$'],'Units','Normalized',...
    'FontSize',30,'Interpreter','LaTeX')
text(-0.1,-0.3,'(b)','Units','Normalized','FontSize',30,'Interpreter','LaTeX')
set(gca,'FontSize',24,'TickLabelInterpreter','LaTeX')
if save_plots
    feval('export_fig',[file_path,'Figure3b.pdf'],'-pdf')
end
drawnow